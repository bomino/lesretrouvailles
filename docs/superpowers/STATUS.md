...
| Promotions | Members-only class-roster archive (`/promotions/`) — 352 entries / 11 classes from the 1980-82 rosters, guarded « C'est moi » claim flow, `noindex`. Source data gitignored: the repo is public. | Complete (2026-07-12) | logged in post-launch polish below |
| Pre-launch hardening | Security + tech-debt sweep before go-live — Django 5.0 (EOL) → 5.2 LTS, token redaction in logs, contact visibility made opt-in, EXIF hole in self-service upload closed, ~30 review findings triaged and closed. | Complete (2026-07-13) | logged in post-launch polish below |
| Review-findings sweep | Top findings from the 2026-08-01 full-codebase review — vouch state-machine guard (RGPD escape), cron failure isolation, `make docker-run` boot repair, allauth proxy-aware rate limits, gestion flash/atomicity/PurgeIncomplete fixes. | Complete (2026-08-01) | [plan](plans/2026-08-01-review-findings.md) |
| Review-tail batch | The medium/low tail of the same review, nine grouped commits — emails out of transactions, vouch/approve race locks, admin-action honesty, questionnaire + duplicate-signup guards, undecided-application retention + digest idempotency + audit-trail coverage, members robustness (session sweep on purge, CSV append, byte-sniffed uploads), console/template hygiene, memoriam/memoires guards, infra currency (Node 22, `.dockerignore` privacy patterns, report-only CSP). | Complete (2026-08-02) | [plan](plans/2026-08-01-review-tail.md) |
| Review mediums 2026-08-22 | The six priority Mediums of the 2026-08-22 full-codebase review, one TDD'd commit each — Referer-atom redaction in the gunicorn access log, prod basic-auth gate defaults off, self/superuser suspend guard in the console, `AdminApplication.status` read-only + requeue action, FOR UPDATE on the application row in vouch/questionnaire POSTs, removal-request rate limit spent only on valid submissions. | Complete (2026-08-22) | — |
| Sentry error tracking | Opt-in `SENTRY_DSN` on prod-shaped settings, every event scrubbed through the same token redaction as the access log (`alumni/sentry.py`), `send_default_pii=False`, no tracing; report-only CSP gains a `report-uri` collector derived from the DSN (review L42). Chosen over the other 2026-08-22 enhancement ideas because the August cron outage proved nothing pushes failures at the owner. | Complete (2026-08-23) | — |
...
| 2026-05-10 | `feat/active-nav-indicator` (1 commit) | `2728f73` | **Active-page indicator across all navigation surfaces.** User report: nothing on the frontend signaled which page they were on. Adds `core/templatetags/active_link.py` with two simple_tags — `{% active_class prefix [exact=True] %}` returns `bg-base-200 text-tertiary font-medium` when `request.path` matches (prefix-match by default; exact mode for the gestion dashboard root link so it doesn't stay active on every /gestion/* page), and `{% active_aria prefix %}` returns ` aria-current="page"` as a SafeString. Applied to `templates/base.html` (desktop navbar 6 links + mobile dropdown 7 links) and `gestion/templates/gestion/base.html` (subnav 4 links — replaces the ad-hoc `resolver_match.url_name` check on Tableau de bord). Two-tier signal in /gestion/: the top-level "Gestion" link in the main navbar stays active on every /gestion/ subpage (prefix), the subnav highlights the specific section. 16 new tests (12 unit + 4 integration). Suite grew 773 → 789. |
| 2026-05-10 | `feat/gestion-souvenirs` (18 commits) | feat: `2c75548` + `20fa72f` + `50327fe` + `7d88df9` + `ce5476d` + `5d1d1b6`; cloudinary: `577a64c` | **Co-admin Mur des souvenirs management on `/gestion/`** — full parity with super-admin: list view with status filter chips (Toutes/Publiées/Brouillons) + accent-insensitive `?q=` search over caption+location + pagination at 12, create view with Cloudinary upload + atomic AuditLog write, edit view with photo replace (select_for_update lock + transaction.on_commit cleanup of old blob) + no-op detection via WATCH_FIELDS snapshot + status-flip emission rules, publish/unpublish toggle view. 4 routes under `/gestion/souvenirs/`, no detail page (edit IS the photo-viewing surface at size=400). New 4th dashboard KPI tile (`draft_memories` count, grid bumps to `md:grid-cols-2 lg:grid-cols-4`, renders count even at 0). Subnav gains "Souvenirs" link after "Cooptations". 6 new `?flash=` keys (created/updated/published/unpublished/noop/bad_status) wired into `gestion/base.html`. 4 new `memoires.memory.*` actions in `AuditLog.ACTION_CHOICES` (migration `members 0021_alter_auditlog_action`). **Material side benefit: server-side EXIF strip via Pillow at upload time** in `alumni.cloudinary._strip_exif_metadata`, integrated into `RealCloudinary.upload_file` — strips EXIF/XMP/IPTC from ALL future uploads (member admin photos, memoires, memoriam) by re-encoding via Pillow without the `exif=` kwarg. `fl_strip_profile` added to both `memory_thumbnail_url` and `memory_full_url` delivery transforms for defense-in-depth on pre-phase photos. Pre-phase Cloudinary originals retain their EXIF in storage; reaching them requires constructing the raw `cloudinary.com/<cloud>/image/upload/<public_id>` URL — `restrip_existing_memories` management command becomes Phase 2. `DATA_UPLOAD_MAX_MEMORY_SIZE` raised to 10 MB. Caption XSS regression test (3 surfaces: public `/souvenirs/`, `/gestion/souvenirs/` list, edit form) locks "caption is plain text only". Spec + plan: [`specs/2026-05-10-gestion-souvenirs-design.md`](specs/2026-05-10-gestion-souvenirs-design.md) + [`plans/2026-05-10-gestion-souvenirs.md`](plans/2026-05-10-gestion-souvenirs.md). Suite grew 707 → 773 (+66 tests across 6 new test files + dashboard extension). Internal capability, no version tag. |
...
| 2026-07-12 | `feat/promotions` | `4d1af1f` | **Promotions — members-only class-roster archive.** Two Excel workbooks from a classmate (6ème 1980-81 classes A-F, 6ème 1981-82 classes A-E) became 352 `ClassRosterEntry` rows across 11 classes. The original ask was "populate the Annuaire", which is not possible and should not be: `Member.user` is a non-nullable OneToOne, and the files carry no phone, no email, no consent — filling the directory would mean fabricating 335 login accounts for people who never registered. Instead the names got their own home at `/promotions/` (login-gated, `noindex`), where alumni browse their real class list, see which classmates already registered, and claim their own row via a guarded « C'est moi ». **The claim guard matters:** an unguarded claim lets any member attach any classmate's name to their profile, so `can_claim` requires ≥2 shared normalized name tokens plus a `select_for_update` row lock and a one-per-school-year cap. FK is `CASCADE` (not `SET_NULL`) so an RGPD purge actually erases a claimed row instead of leaving the person's full name behind. Idempotence keys on `source_ref` (`<file>:<sheet>:<row>`), not on names — 20 rows have a blank surname and would have collided. **Data finding:** the 80-81 workbook's headers are mislabeled (`Noms \| Prénoms`); all 12 people present in both files share the same physical column order, and the owner's own row (`Mahamadou \| Laouali`, «Bomino») confirms column B = given name. Trust the data, not the header. Roster data is **never committed** — the repo is public and these are 335 real living people; `*.xlsx` + `/private-data/` gitignored in `4961f34`. |
| 2026-08-01 | `fix/review-findings` (7 commits) | `90415e8`…`47a6314` | **Review-findings sweep** — top items from the 2026-08-01 five-agent full-codebase review, in priority order, one TDD'd commit each ([plan](plans/2026-08-01-review-findings.md)). (1) `parrain_vouch_view` gains the application-decided guard the questionnaire view always had: a late vouch could flip a rejected application out of `status='rejected'` (escaping the 180-day retention purge) or make an approved one re-decidable. (2) `_send_j7_reminders` isolates per-item failures — one Resend outage used to abort the entire daily cron, silently skipping that day's expiries and retention purges. (3) `make docker-run` boots again: docker-compose.yml opts out of the localhost-SITE_URL guard via new `SITE_URL_ALLOW_LOCAL` + console email backend; the guard now also catches `http://127.0.0.1`. (4) `ALLAUTH_TRUSTED_PROXY_COUNT = 1` — allauth's login/reset throttles were keying on `REMOTE_ADDR` (Railway's proxy), collapsing every visitor into one shared bucket. (5) gestion flash banners are page-owned (no more duplicate memory-worded banner + raw-token reflection on member/cooptation flows). (6) `PurgeIncomplete` is caught in both RGPD purge front-ends (admin action + CLI) instead of surfacing as a 500/traceback. (7) `member_status_view` and both gestion `save_with_audit` forms are atomic — suspend could previously half-apply, leaving a "suspended" member whose account still logged in. Suite grew 954 → 970 (+16 tests). Remaining review findings (medium/low tail + the email-only-parrainage product question) logged in the review conversation, not yet scheduled. |
| 2026-08-02 | `fix/review-tail` (11 commits) | `537a330`…`ae5c717` | **Review-tail batch** — the medium/low tail of the 2026-08-01 review in nine grouped TDD'd commits ([plan](plans/2026-08-01-review-tail.md)). Cooptation: candidate emails moved out of the vouch transaction (Resend outage no longer rolls back a recorded response); FOR UPDATE locks close the concurrent-vouch stall and the approve TOCTOU; `message_user` stops swallowing real errors; the dead admin reject-reason read removed; resend-link matches by username; all-blank questionnaire submissions rejected + zero-active-questions dead-end fixed; duplicate in-flight signups blocked; **undecided applications now purge on the same 180-day clock as rejections** (they kept PII forever); quarterly digest fires on a day 1-7 window with a `ghost.digest.sent` marker (a missed cron on the 1st used to skip a whole quarter); `cooptation.application.approved/rejected/purged` audit actions (migration `members 0025`); smoke-test cleanup no longer strands parrains. Members: RGPD purge sweeps live sessions (they have no FK to User and survived 90 days); removal-confirm POST + `unclaim_entry` take row locks; magic-links CSV appends instead of truncating; roster photo upload moved out of the per-row transaction; `photo_upload_view` sniffs real bytes via Pillow; `create_member` tolerates duplicate emails. Gestion/core/aide: `q` URL-encoded in list hrefs; dead `nonce=""` removed (2×); basic-auth uses `compare_digest`; `landing_placeholder` deleted; the /aide/ guide link is one blocktrans again. Memoriam/memoires: nomination quota consumed only after the Member resolves; /admin/ hard deletes write `memoires.memory.deleted`/`memoriam.entry.deleted` audit rows; both admin upload forms get the 8 MB + MIME guards. Infra: Node 20 (EOL) → 22 in Dockerfile + CI; `.dockerignore` gains `private-data/` + `*.xlsx` + six test dirs; vestigial `COPY DESIGN.md` dropped; CI concurrency/timeouts/pip-cache key; compose app healthcheck; stray `5.2` file deleted; **report-only CSP** middleware on staging/prod (observation before enforcement). Two review findings were *disproven* during TDD and locked in as regression tests instead: LIKE-wildcard escaping in search (Django already escapes) and the bleach pipeline's strip-vs-escape behavior. Also fixed en route: **the local venv was still on Django 5.0.14** (editable install predating the 5.2 upgrade) — reinstalled so local runs test what prod ships. Suite grew 970 → 1011 (+41 tests). Deliberately deferred: dependency lock (needs a Linux-side pip-tools workflow), bleach→nh3, staff session cap, CSP enforcement flip, actions SHA-pinning, H2 email-only parrainage (product decision). |
| 2026-08-22 | `fix/review-2026-08-22-mediums` (6 commits) | `1a9f7fe`…`2847444` | **Review mediums** — the top six of the 2026-08-22 review (60 findings, each re-verified against source before fixing: 53 confirmed, 5 partial, 2 refuted — L34 manifest 404 and L39 tracked `.env`). (1) `_URL_ATOMS` gains `"f"`: gunicorn's default access_log_format prints the Referer, so every asset request after a token page re-leaked what 7a325c1 hid. (2) `prod.py` defaults `BASIC_AUTH_REQUIRED` to False — staging's True would 401 the whole site behind green healthchecks. (3) `member_status_view` refuses to suspend oneself or a superuser (a lockout with no UI recovery); button hidden on those rows. (4) `AdminApplication.status` is read-only in `/admin/`; a manual `rejected`/`purged` skipped the services and kept PII forever. The guide's hand-edit remediation becomes a « Remettre en file de revue » action (cooptation_pending → awaiting_admin only). (5) Vouch and questionnaire POSTs lock the application row and re-run the decided gate inside the transaction — a concurrent admin decision was silently overwritten back to awaiting_admin. (6) Removal-request rate limit moves from the decorator to `is_ratelimited(increment=True)` after `validate_email` — malformed/probing POSTs no longer burn the CGNAT-shared 5/h bucket. Three reviewer-proposed fixes were **not** adopted as written: plain read-only `status` (strands stalled apps), republish re-firing the memoriam email (would re-notify ~200 members), hashing `old_username`/`new_username` (the only recovery record for a mistyped number). Suite grew 1011 → 1028 (+17 tests). Tail (M7–M14, Lows) scheduled as a follow-up batch; M11 (charter promises RGPD export) needs a product decision. |
| 2026-08-23 | `fix/email-backend-boot-check` (2 commits) | `ddfd170`…`564b66d` | **Cron outage + latent email outage.** Both Railway cron services had been dying at settings import on every scheduled run — `media-backup-cron` since at least 07-26 (no `SITE_URL` → localhost guard), `cooptation-cron` since at least 08-04 (`RESEND_API_KEY` present but empty; `SITE_URL` pointed at *staging*). The `staging.py` fail-fast guards from `b4d86d8` were right; the cron services' variables never got them, and `staging-deploy.md` claimed the cron "shares" the app's env (Railway services inherit nothing). Fixed in Railway config with reference variables (`${{lesretrouvailles.*}}`), verified by a `backup_media` run (`17 skipped, already backed up`) and a `process_cooptation_deadlines` run (`Done. reminders=0 … audit_purged=0`) on the repaired services. Found en route: the **web service's `EMAIL_BACKEND` was `django_resend.EmailBackend`**, a module absent from the image — every real send would have raised `ModuleNotFoundError`; the name-based guard could not see it. Corrected to `alumni.email.ResendBackend`; `staging.py` now `import_string()`s the backend at boot (+1 test, suite 1029). Runbooks corrected (`staging-deploy.md`, `restore.md` §1.3). |
| 2026-08-23 | `feat/sentry` (2 commits) | `8e1adf9`…`63a5b3f` | **Sentry error tracking.** Assessment of `docs/superpowers/2026-08-22-enhancement-ideas.md` against the codebase: prod holds ~18 members and the roster import has never run, so engagement features (P9.3 auto-DM, events, reactions, PWA) are premature; the one thing worth building now is error push — LOGGING is console-only, no `ADMINS`, and the cron outage went unseen for a month. `staging.py` inits `sentry_sdk` only when `SENTRY_DSN` is set (Docker build stays green); `alumni/sentry.py::scrub_event` runs `redact_tokens` over request URL/query, transaction, breadcrumbs and exception values and drops cookies/Authorization — verified through the real SDK with a capturing transport. `csp_report_uri()` derives Sentry's security endpoint for the report-only CSP (L42). Runbooks: `SENTRY_DSN` goes on the app service **and both crons**; launch checklist gains an "Error alerts" line. Corrected priority for the ideas doc: run the launch → Sentry → start Meta paperwork (P9.1 §E.4, calendar-bound) → Events/RSVP once members exist → review tail incl. the M11 export decision → P9.3 when approvals land. Suite 1029 → 1038 (+9). |
| 2026-08-23 | `docs/launch-runbook-currency` + `docs/pilot-repair-findings` | `70ad6bd`… | **Launch runbook brought in line with production.** Read-only prod inventory: 18 real members (May 2026, hand-created, non-digit usernames; 11 never activated, 16 without `Member.whatsapp`) — so Step 0's "delete the seed fixtures" was a data-loss trap (demoted to staging-only), and the pilot is a **repair**, not an import: re-importing hand-created accounts would duplicate them because idempotency keys on digit usernames. Built `private-data/pilot_repair.csv` (gitignored) mapping each member to the digit username + WhatsApp number recovered from `roster_pilot.xlsx`, with the judgment rows flagged (US numbers missing the leading 1, three members with no phone). Also fixed: the two `shell -c` commands that `railway ssh` breaks, and Step 4's env block (jq-free, full guard-satisfying variable list). |
| 2026-07-12 | `fix/pre-launch-review-batch-1` (many commits) | through `5127097` | **Pre-launch hardening sweep.** Two external reviews (one internal `/code-review`, one Codex) plus a self-audit produced ~30 findings; the live ones are closed. Highlights: **Django 5.0 (EOL) → 5.2 LTS** with open advisories, and auth tokens no longer leak into logs (`alumni/logging.py` — `TokenRedactingFilter` + `RedactingGunicornLogger`) (`7a325c1`). **F-02 contact visibility is opt-IN** — `Member.show_email` / `show_whatsapp` defaulted to `True` while every guide promised the opposite; data migration flipped 18 existing members (`47a19aa`). **F-03/F-06** — the self-service photo upload bypassed the EXIF strip (GPS coordinates of members' homes reaching Cloudinary), and an RGPD purge was deleting gallery photos that belonged to the community, not the member (`1043d04`). Plus rate-limiting on anonymous AuditLog writes, rightmost-XFF trust, cooptation state gaps, French error pages, and a `{# #}`-does-not-span-lines fix that had been leaking developer notes into `500.html` before `<html>` (`1e18ea2`). Audit docs (`CODE_REVIEW.md`, `TECH_DEBT_AUDIT.md`) are **deliberately gitignored** — they enumerate still-open findings with file:line detail and the repo is public (`81a2882`). |
| 2026-07-13 | `fix/hygiene` | `b4d86d8` + `ecc9bae` | **Remaining tech-debt findings closed.** Three were real bugs, not tidying. **F-12** — `PublicSearchEntry.is_published` required 2 admins while the landing page, cron, admin filter and audit trail all use the P4d one-admin rule; the ghost list would have rendered empty. The test "protecting" it was itself asserting the dead rule. **F-22** — the gestion console matched the whole query as a *single substring*, so an operator typing a full name got zero hits while the same search worked in the Annuaire; both now share `members/search.py`, and the staff variant also searches `Member.whatsapp`. **F-25/F-26** — photo replacement deleted the old Cloudinary asset *inside* the transaction, so an outage rolled back a profile the member had already saved; now deferred to `on_commit`. **F-20/F-21 were worse than reported:** the three "search" indexes from migration `0004` could never be used by Postgres at all (they index `unaccent_immutable(...)` while `search.py` emits `UNACCENT(...)`, and a btree cannot serve a leading-wildcard LIKE regardless) — dropped in migration `0024`; `unaccent_immutable` + `pg_trgm` retained so the real GIN fix stays a one-file change, deliberately not done pre-launch on the hot path of the most-used page. Also: F-29 fail-fast on a localhost `SITE_URL` / empty `RESEND_API_KEY` (which **broke the Docker build on first push** — `compilemessages` loads staging settings with no config; fixed in `ecc9bae` by giving the build dummies, as it already does for `SECRET_KEY`), F-14 packaging omitted four live apps, F-23/F-24 roster-import phone + MIME handling, F-10/F-11 memoriam audit actor, F-28 staging parity, F-31 query-count guardrails. |
**Test suite trajectory:** 492 (v1.0.0) → 513 (class fix) → 516 (mobile nav) → 517 (cloudinary fix) → 519 (public-search-entry UX) → 521 (admin nav link) → 522 (favicon) → 530 (optional classes) → 622 (Gestion v1) → 650 (post-Gestion polish) → 666 (P8 `/aide/` end-to-end) → 683 (P8 `/annuaire/` upgrades) → 697 (P8.1 `/guide/` page) → 707 (handbook pipeline) → 773 (gestion souvenirs) → 789 (active-nav indicator) → 869 (promotions) → 931 (pre-launch hardening sweep) → **954** (hygiene batch). New tests added with each fix; no regressions introduced.
