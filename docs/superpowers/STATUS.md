...
| 2026-05-08 | `feat/member-guide-page` | _this commit_ | **P8.1** — Public `/guide/` page that renders the canonical `docs/guides/guide_membre.md` server-side (single read at module load via `aide/guide.py`, markdown + bleach pipeline mirrors `/aide/` answers, `toc` extension auto-generates anchor IDs). Sticky-TOC desktop layout / accordion-TOC mobile layout. CTA from the `/aide/` hero, link in the footer (also adds `/aide/` to the footer for completeness), and the `/aide/` empty-state retargeted from GitHub to `/guide/`. Loud-failure-mode: missing markdown file logs WARNING and serves placeholder; unit test asserts `_GUIDE_PATH.exists()` so a future rename or Dockerfile drift fails CI. Dockerfile gains `COPY docs/guides/ ./docs/guides/` (runtime stage only — caught at the bottom of the path-discovery list, same gotcha class that bit P8). 14 new tests (4 view-level, 10 module-level on render output, TOC structure, anchor consistency, missing-file fallback). Suite grew 683 → ~697. |
...
**Test suite trajectory:** 492 (v1.0.0) → 513 (class fix) → 516 (mobile nav) → 517 (cloudinary fix) → 519 (public-search-entry UX) → 521 (admin nav link) → 522 (favicon) → 530 (optional classes) → 622 (Gestion v1) → 650 (post-Gestion polish) → 666 (P8 `/aide/` end-to-end) → 683 (P8 `/annuaire/` upgrades) → 697 (P8.1 `/guide/` page). New tests added with each fix; no regressions introduced.
