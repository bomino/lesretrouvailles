...
This file is loaded by Codex when working in this repository. It captures the load-bearing conventions and gotchas that aren't obvious from reading the code.
...
Earlier in the project I (Codex) was rate-limited for piping production Cloudinary credentials into a local Python process. **Don't do that.**
...
- The `Bash` tool in this Codex env is Git Bash, not WSL. Most things work; Postgres connections to localhost work via the Docker port mapping.
